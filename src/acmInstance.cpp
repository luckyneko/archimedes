...
namespace
{
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL vulkanValidationCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData)
	{
		if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
			...
			fprintf(stderr, "validation layer [warn]: %s\n", pCallbackData->pMessage);
		return VK_FALSE;
	}
#endif
	...
	bool isExtensionAvailable(const std::vector<VkExtensionProperties>& availableExtentions, const char* extName)
	{
		auto iter = std::find_if(availableExtentions.begin(), availableExtentions.end(),
								 [extName](const VkExtensionProperties& extension)
								 { return (strcmp(extName, extension.extensionName) == 0); });
...
acm::Instance::Instance(const char* appName, const acm::Version& appVer)
{
	...
	{
		m_error = acm::Error(std::string("Failed to create instance: ") + vkResultStr(instanceRes));
		return;
	}
	...
	{
		m_error = acm::Error(std::string("Failed to set up debug messenger: ") + vkResultStr(debugMessengerRes));
		return;
	}
#endif
	...
	for (size_t deviceIdx = 0; deviceIdx < devices.size(); ++deviceIdx)
	{
		...
		vkGetPhysicalDeviceFeatures(gpu.device, &features);
...
acm::Surface acm::Instance::createSurface(VkSurfaceKHR surface)
{
	return acm::Surface(*this, surface);
}

acm::Device acm::Instance::createDevice(const acm::GPU& gpu, uint32_t queueIdx)
{
	return acm::Device(*this, gpu, queueIdx);
}
