...
namespace acm::vulkan
{
	...
	RenderTarget::RenderTarget(Device& owner, VkImage image, acm::Format format, acm::Extent2D extent, bool depth, acm::SampleCount samples)
	{
		...
		m_owner = &owner;
		m_colorImage = image;
		m_colorFormat = toVk(format);
		m_extent = extent;
		m_depth = depth;
		m_depthFormat = depth ? toVk(acm::Format::D32_Sfloat) : VK_FORMAT_UNDEFINED;
	...
	acm::Extent2D RenderTarget::extent() const
	{
		return m_extent;
	}
	...
	bool RenderTarget::multisampled() const
	{
		return m_samples != VK_SAMPLE_COUNT_1_BIT;
	}

	VkFormat RenderTarget::colorFormat() const
	{
		return m_colorFormat;
	}

	VkFormat RenderTarget::depthFormat() const
	{
	...
	bool RenderTarget::beginRendering(VkCommandBuffer commandBuffer, float r, float g, float b, float a) const
	{
	...
	void RenderTarget::endRendering(VkCommandBuffer commandBuffer) const
	{
		if (!commandBuffer)
			return;
	...
	bool RenderTarget::createTransientAttachments(Device& owner)
	{
		const bool isMultisampled = m_samples != VK_SAMPLE_COUNT_1_BIT;
		if (m_depth)
		{
			if (isMultisampled)
			{
		...
		if (isMultisampled && !createAttachment(owner, m_msaaColor, m_colorFormat, m_extent, m_samples, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_IMAGE_ASPECT_COLOR_BIT))
			return false;
		return true;
	}

	bool RenderTarget::createAttachment(Device& owner, Attachment& attachment, VkFormat format, acm::Extent2D extent, VkSampleCountFlagBits samples, VkImageUsageFlags usage, VkImageAspectFlags aspect)
	{
	...
	VkImage RenderTarget::colorAttachmentImage() const
	{
		return m_samples != VK_SAMPLE_COUNT_1_BIT ? m_msaaColor.image : m_colorImage;
	}

	VkImageView RenderTarget::colorAttachmentView() const
	{
		return m_samples != VK_SAMPLE_COUNT_1_BIT ? m_msaaColor.view : m_colorView;
	}

	VkImage RenderTarget::depthAttachmentImage() const
	{
		if (!m_depth)
	...
	void RenderTarget::recordBeginTransitions(VkCommandBuffer commandBuffer) const
	{
	...
	void RenderTarget::recordEndTransition(VkCommandBuffer commandBuffer) const
	{
	...
	void RenderTarget::destroyAttachment(Device& owner, const Attachment& attachment)
	{
		if (attachment.view)
