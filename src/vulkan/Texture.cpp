...
namespace acm::vulkan
{
	...
	Texture::Texture(Device& owner, acm::Format format, acm::Extent2D extent, bool mipmapped, bool storage)
	{
		...
		m_format = format;
		m_extent = extent;
		const VkFormat vkFormat = toVk(format);
	...
	acm::Format Texture::format() const
	{
		return m_format;
	}

	acm::Extent2D Texture::extent() const
	{
		return m_extent;
	}
	...
	VkImageView Texture::vkImageView() const
	{
		return m_imageView;
	}
	...
	void Texture::recordCopyToBuffer(VkCommandBuffer commandBuffer, const Buffer& buffer) const
	{
	...
	void Texture::recordTransition(VkCommandBuffer commandBuffer, acm::ImageLayout from, acm::ImageLayout to) const
	{
		const VkLayoutInfo source = toVk(from);
		const VkLayoutInfo destination = toVk(to);
	...
	acm::Error Texture::upload(const void* pixels, size_t size)
	{
		...
		if (uploadedMipLevels > 1)
		{
			...
			vkGetPhysicalDeviceFormatProperties2(owner().vkPhysicalDevice(), toVk(m_format), &properties);
	...
	bool Texture::isDepthFormat(acm::Format format)
	{
		return format == acm::Format::D32_Sfloat || format == acm::Format::D24_Unorm_S8_Uint;
	}

	uint32_t Texture::computeMipLevels(acm::Extent2D extent)
	{
		uint32_t levels = 1;
	...
	void Texture::transition(VkCommandBuffer commandBuffer, VkImage image, uint32_t baseMip, uint32_t levelCount, VkImageLayout oldLayout, VkImageLayout newLayout, VkAccessFlags2 sourceAccess, VkAccessFlags2 destinationAccess, VkPipelineStageFlags2 sourceStage, VkPipelineStageFlags2 destinationStage)
	{
		VkImageMemoryBarrier2 barrier = {};
		barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2;
