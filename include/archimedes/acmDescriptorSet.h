...
namespace acm
{
	...
	public:
		...
		acm::backend::DescriptorSet* backend() const;
	...
	private:
		...
		DescriptorSet(acm::ResourceRef<acm::backend::DescriptorSet> resource);
		explicit DescriptorSet(acm::Error error);

		acm::ResourceRef<acm::backend::DescriptorSet> m_resource;
		acm::Error m_error;
	};
} // namespace acm
